...
const PROVIDERS = {
  ...
  gemini:  { label: 'Gemini (Google)',    model: 'gemini-2.5-pro',       keyName: 'gemini_api_key',    priceIn: 1.25, priceOut: 10, pdf: true },
...
async function callClaude({ apiKey, system, text, pdfBase64, zodSchema, maxTokens }) {
...
async function callGemini({ apiKey, system, text, pdfBase64, zodSchema }) {
...
async function callMistral({ apiKey, system, text, pdfBase64, zodSchema }) {
...
// Appel structuré générique, avec une relance en cas de JSON hors schéma
async function aiStructured({ provider, apiKey, system, text, pdfBase64, zodSchema, maxTokens }) {
  const fn = provider === 'gemini' ? callGemini : provider === 'mistral' ? callMistral : callClaude
  let lastErr
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      return await fn({ apiKey, system, text, pdfBase64, zodSchema, maxTokens })
    } catch (e) {
      lastErr = e
      // Ne relancer que les réponses illisibles/hors schéma, pas les erreurs d'auth/quota
      if (!/illisible|hors schéma|vide/i.test(e.message)) throw e
    }
  }
  throw lastErr
}
...
async function extractPrices({ provider = 'claude', apiKey, kind, base64, text, filename, lots, hints }) {
  if (kind === 'pdf') {
...
async function chiffrerLignes({ provider = 'claude', apiKey, lignes, projet }) {
